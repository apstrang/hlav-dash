...
const Overview = () => {
  ...
  const getWeather = (value: Date) => {
    switch (value.getDay()) {
      case 0:
        return `<img class="weather-image" src=${base_path}/images/schedule/weather-clear.svg alt="Clear Weather"/>`;
      case 1:
        return `<img class="weather-image" src=${base_path}/images/schedule/weather-clouds.svg alt="Clouds Weather"/>`;
      case 2:
        return `<img class="weather-image" src=${base_path}/images/schedule/weather-rain.svg alt="Rain Weather"/>`;
      case 3:
        return `<img class="weather-image" src=${base_path}/images/schedule/weather-clouds.svg alt="Clouds Weather"/>`;
      case 4:
        return `<img class="weather-image" src=${base_path}/images/schedule/weather-rain.svg alt="Rain Weather"/>`;
  ...
  */
  ...
  let generateEvents = (): Record<string, any>[] => {
    ...
    let currentTimezone: never;
    if (typeof window !== 'undefined') {
      currentTimezone = timezone.getLocalTimezoneName() as never;
    } else {
      currentTimezone = 'UTC' as never;
    }
  ...
  const btnClick = () => {
    ...
    if (settingsPanel && settingsPanel.classList.contains('hide')) {
  ...
  if (action === 'eventCreated') {
    const { error } = await supabase.from('events').insert({
      ...
      owned_by: data.EmployeeId,
      color: data.Color ?? '#91C6E0', // default if needed
    })
    if (error) console.error('Insert error:', error)
  }

  if (action === 'eventChanged') {
          ...
          <div className='schedule-overview'>
            <AppBarComponent colorMode="Primary">
              ...
              <div className='control-panel import-button'>
                <UploaderComponent id='fileUpload' type='file' allowedExtensions='.ics' cssClass='calendar-import' buttons={typeof document !== 'undefined' ? { browse: importTemplateFn({ text: 'Import' })[0] as HTMLElement } : undefined}
                  multiple={false} showFileList={false} selected={(onImportClick)} created={createUpload} />
              </div>
              <div className='control-panel calendar-export'>
            ...
            <div className='overview-content'>
              <div className='left-panel'>
                <div className='overview-scheduler'>
                  <ScheduleComponent id='scheduler' cssClass='schedule-overview' ref={scheduleObj} width='100%' height='100%' currentView={currentView} group={{ resources: ['Employees'] }} timezone='UTC' eventSettings={{ dataSource: events, fields: { color: 'Color' } }} dateHeaderTemplate={dateHeaderTemplate} eventRendered={(args) => { if (args.data.Color) { args.element.style.backgroundColor = args.data.Color, args.element.style.color = 'black', args.element.style.borderLeft = '5px solid blue' } }} group={{ resources: ['Employees'], byGroupID: false, allowGroupEdit: true }} actionComplete={handleActionComplete}>
                    <ResourcesDirective>
                      <ResourceDirective field='EmployeeId' title='Employees' name='Employees' allowMultiple={false} dataSource={profiles} textField='Text' idField='Id' />
                    </ResourcesDirective>
                    <ViewsDirective>
